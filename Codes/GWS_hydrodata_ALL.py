...
class DataforIndividual():
    ...
    def load_forcing_data(self, working_path, huc_id, basin_id):
        ...
        forcing_data['GW(mm)'] = (65 - forcing_data['GW(feet)']) / 5 
    ...
    def load_data(self):
        ...
        merged_data = merged_data[(merged_data['date'] >= datetime(1980, 9, 1)) &
                                  (merged_data['date'] <= datetime(2012, 9, 30))]
        merged_data = merged_data.set_index('date')
        pd_data = merged_data[['prcp(mm/day)', 'tmean(C)', 'dayl(day)', 'srad(W/m2)', 'vp(Pa)', 'flow(mm)', 'GW(mm)' ]]
